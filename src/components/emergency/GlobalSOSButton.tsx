...
import { AlertOctagon, PhoneCall, X, MapPin, Send, Loader2 } from 'lucide-react';
...
export const GlobalSOSButton: React.FC = () => {
  ...
  const handleTriggerSOS = async () => {
    ...
    // Acquire GPS location gracefully
    const loc = await EmergencyService.getCurrentLocation(5000);
    setLocation(loc);
    setIsLocating(false);
    ...
    await EmergencyService.recordEmergencyEvent(
      ...
      loc,
      ['Manoj Kumar (Son): +91 94471 98765', 'Suma Varghese (ASHA): +91 98460 12345']
    );
    setSosSent(true);
  };

  const encodedSms = EmergencyService.formatEmergencySms(
    user?.fullName || 'AroggyaGram Citizen',
  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed bottom-[calc(5.5rem+env(safe-area-inset-bottom,0px))] right-5 z-40 flex items-center justify-center gap-2 bg-[#D62828] hover:bg-[#B51E1E] text-white px-5 py-3.5 rounded-full shadow-[0_6px_20px_rgba(214,40,40,0.45)] border-2 border-white transition-all active:scale-95 font-bold text-base tracking-wide"
      >
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 bg-[#8F1616]/95 backdrop-blur-md text-white flex flex-col justify-between p-6 overflow-y-auto">
          ...
          <div className="my-6 bg-black/30 rounded-2xl p-5 border border-white/20 space-y-3">
            ...
            {isLocating ? (
              ...
              <div className="space-y-1">
                ...
                <p className="text-sm text-white/70">
                  Responders will contact you directly on your primary registered telephone.
                </p>
              </div>
            )}
            ...
            {sosSent && (
              <div className="bg-emerald-900/60 border border-emerald-400/40 rounded-xl p-3 text-emerald-100 text-sm font-medium">
                ✓ SOS broadcast logged to network. Caregivers notified.
              </div>
            )}
          </div>
          ...
          <div className="space-y-4 my-auto">
            ...
            <a
              href="tel:+919447198765"
              className="w-full flex items-center justify-center gap-3 bg-emerald-600 hover:bg-emerald-500 text-white py-5 px-6 rounded-2xl font-black text-xl shadow-xl transition-all active:scale-98 border-2 border-emerald-300"
            >
              <PhoneCall className="w-7 h-7" />
              <span>Call Caregiver (Manoj)</span>
            </a>

            {/* Action 3: Send SMS with GPS link */}
            <a
              href={`sms:+919447198765?body=${encodedSms}`}
              className="w-full flex items-center justify-center gap-3 bg-white/20 hover:bg-white/30 text-white py-4 px-6 rounded-2xl font-bold text-lg border border-white/40 transition-all active:scale-98"
            >
              <Send className="w-6 h-6" />
              <span>Send SMS with Live Location Link</span>
            </a>
          </div>
